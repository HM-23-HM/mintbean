...
const mapDispatchToProps = {
    dispatchSendCards: (asker, being_Asked, matchingCards) => sendCards(asker, being_Asked, matchingCards),
...
const Options = (props) => {
    ...
    const P1_ask = (selectedOption) => {
        console.log(`Selected the following option: ${selectedOption} to ask ${props.beingAsked}`)
        let option = selectedOption
        let asker = 'P_1'
        let being_Asked = props.beingAsked
    
        let beingAsked_Cards = mapAItoCards.get(being_Asked)
        let matchingCards = beingAsked_Cards.filter((card) => card.symbol == option)
